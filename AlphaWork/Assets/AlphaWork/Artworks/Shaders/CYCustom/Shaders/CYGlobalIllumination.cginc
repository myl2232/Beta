...
inline UnityGI UnityGI_Base(UnityGIInput data, half occlusion, half3 normalWorld, out half3 lightmapBakedColor)
{
    lightmapBakedColor = 0;//add myl
    UnityGI o_gi;
    ResetUnityGI(o_gi);
    ...
    o_gi.light.color *= data.atten;
        ...
        lightmapBakedColor = bakedColor;//add myl
...
{
    half3 lightmapBakedColor;//add myl
    return UnityGI_Base(data, occlusion, normalWorld, lightmapBakedColor);
}

inline UnityGI UnityGlobalIllumination (UnityGIInput data, half occlusion, half3 normalWorld, Unity_GlossyEnvironmentData glossIn)
{
    half3 lightmapBakedColor;//add myl
    UnityGI o_gi = UnityGI_Base(data, occlusion, normalWorld, lightmapBakedColor);
    half luminance =  0.212 * lightmapBakedColor.r + 0.715 * lightmapBakedColor.g + 0.07 * lightmapBakedColor.b;
	luminance = pow(luminance, _GISpecularPower) * _GISpecularScale;
    o_gi.indirect.specular = UnityGI_IndirectSpecular(data, occlusion, glossIn) * luminance;
    return o_gi;
}
