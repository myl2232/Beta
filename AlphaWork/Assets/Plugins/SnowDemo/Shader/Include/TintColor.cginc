...
#define TINT_COLOR_INCLUDE  	
	...
	#define VAR_TINT_COLOR_NEED 
		...
		float4 _TintMask_ST;
		half _TintPowerMaxRange;//need setting in properties
	...
	{
		float3 tempColor = grayscaleSrc > grayscaleDes ? inputColor : _TintWeatherColor.rgb;
		return saturate(lerp(tempColor,_TintWeatherColor.rgb,grayscaleDes));
	}
	...
	half3 PerPixelWorldNormal_Weather(float2 uv, half4 tangentToWorld[3])
	{
		#ifdef _NORMALMAP
	...
	#define TINT_CAL_COLOR_MASK_UP_BLEND_ALPHA_4_PBR_NOR(uv,norInW,output) output = Tint_Cal_Color_Mask_Up_Blend_Alpha_4_PBR_Nor(output,uv,norInW);
	STRUCT_TINT_4_PBR Tint_Cal_Color_Mask_Up_Blend_Alpha_4_PBR_Nor(STRUCT_TINT_4_PBR str4PBRInput,float2 uv,float3 wN)
	{		
		half3 normalWorld = PerPixelWorldNormal_Weather(uv, str4PBRInput.tangentToWorld4Tint);
		...
		str4PBRInput.metallicAndSmoothness.x = lerp(str4PBRInput.metallicAndSmoothness.x, _TintMetallic, outValue);
		str4PBRInput.metallicAndSmoothness.y = lerp(str4PBRInput.metallicAndSmoothness.y, _TintSmoothness, outValue);
		str4PBRInput.tintNormal = lerp(str4PBRInput.tintNormal, normalWorld, outValue);
		return str4PBRInput;
	}
