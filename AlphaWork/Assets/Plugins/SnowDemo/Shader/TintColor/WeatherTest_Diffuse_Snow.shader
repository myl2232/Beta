Shader "WeatherTest/Diffuse_Snow"
{
	Properties{
	...
	{   
		...
		Pass     
		{
			...
			float4 frag (v2f i) : SV_Target
			{
				...
				//TINT_CAL_COLOR_MASK_UP_BLEND_ALPHA_FAKENOR(i.uv0.xy,i.worldPos ,tintInput)      
				...
				//float3 tempColor = c.rgb;
				//TintColor
				//非虚化
				//TINT_CAL_COLOR_MASK_DOT_GRAYSCALE_AUTO_UV_CENTER_POS(i.worldPos ,wN,c.rgb) 
				//虚化
				//TINT_CAL_COLOR_MASK_DOT_GRAYSCALE_AUTO_UV_CENTER_POS_BLEND(i.worldPos ,wN,c.rgb)   
				TINT_CAL_COLOR_MASK_UP_BLEND_ALPHA(i.uv0.xy,wN,c.rgb)  
				DECAL_COLOR_WORLD_CENTER(i.worldPos,wN,c.rgb)  
