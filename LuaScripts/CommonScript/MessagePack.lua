...
local packLog = gameLog or print;
local pack_tables_ = {};
table_packer = function(v_type, buffer, v, key)
    if v_type=='table' then
        if pack_tables_[v] then
            packLog("table_packer Error: "..tostring(pack_tables_[v]).." , "..tostring(key or "nil").." , "..dump_obj(v));
			print("FFFFFFFFFFFFFFFFFFFFFF", debug.traceback());
			gameAssert(false, "");
        end
        pack_tables_[v] = key or "nil";
        
	    local save_data = v;
	    if v.base_~=nil then
		    save_data = v.base_;
	    end
	    return packers['_table'](buffer, save_data);
    end
	if v_type=="userdata" then
		print("table_packer... ", key);
		gameAssert(false, "");
	end
    return packers[v_type](buffer, v);
end
packers['map'] = function (buffer, tbl, n)
    ...
    for k, v in pairs(tbl) do
        table_packer(type(k), buffer, k)
        table_packer(type(v), buffer, v, k)
    end
end
...
packers['array'] = function (buffer, tbl, n)
    ...
    for i = 1, n do
        local v = tbl[i]
        table_packer(type(v), buffer, v, i)
    end
end
...
m.log_flag = false;
local root_table = nil;
local call_stack = {};
packers['_table'] = function (buffer, tbl)
	-- local mt = getmetatable(tbl);
	-- setmetatable(tbl, nil);
	local save_data = tbl;
	if tbl.base_~=nil then
		save_data = tbl.base_;
	end
    if m.log_flag then
        if root_table == nil then
            root_table = save_data;
            call_stack = {};
            call_stack[save_data] = "root";
        end
        for k,v in pairs(save_data) do
            if type(k)=="table" then
                if call_stack[k]~=nil then
                    print("msgpack Key recursion: ", k);
                else
                    call_stack[k] = "Key";
                end
            end
            if type(v)=="table" then
                if call_stack[v]~=nil then
                    print("msgpack Value recursion: ", k, v);
                else
                    call_stack[v] = k;
                end
            end
	...
	local ret = packers['_table'](buffer, save_data);

    if m.log_flag then
        if root_table == save_data then
            root_table = nil;
            call_stack = {};
        end
    end
...
function m.pack (data)
    local buffer = {}
    pack_tables_ = {};
    table_packer(type(data), buffer, data)
...
local function unpack_array (c, n)
    local t = {}
--    if unpackers_stack_ then
--        table.insert(unpackers_stack_, t);
--    end
...
function m.unpack (s)
    ...
    if cursor.i < cursor.j then
        gameLog(unpackers_stack_, "m.unpack error: ");
        error "extra bytes"
    end
    return data
end

function m.game_unpack (s)
    checktype('unpack', 1, s, 'string')
    unpackers_stack_ = {};
    local cursor = cursor_string(s)
    local data = unpackers['any'](cursor)
